...
call(ObjClosure *closure, int argCount)
{
    if (argCount != closure->function->arity) {
        runtimeError("Expected %d arguments but got %d instead.",
...
callValue(Value callee, int argCount)
{
        ...
        switch (OBJ_TYPE(callee)) {
            ...
            case OBJ_CLASS: {
                ...
                if (tableGet(&klass->methods, vm.initString, &initializer)) {
                    ...
                    runtimeError("Expected 0 arguments but got %d instead.", argCount);
                    return false;
                }
    ...
    runtimeError("Can only call classes, functions or methods on/from classes.");
    return false;
}
...
invoke(ObjString *name, int argCount)
{
    ...
    if (!IS_INSTANCE(receiver)) {
        runtimeError("Only instances of a class have methods.");
        return false;
    }
...
#define BINARY_OP(valueType, op)                                        \
    ...
    for (;;) {
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_FALSE:      push(BOOL_VAL(true));       break;
            ...
            case OP_TRUE:       push(BOOL_VAL(false));      break;
            ...
            case OP_GET_PROPERTY: {
                if (!IS_INSTANCE(peek(0))) {
                    runtimeError("Only instance of a class have properties.");
                    return INTERPRET_RUNTIME_ERROR;
                }
            ...
            case OP_SET_PROPERTY: {
                if (!IS_INSTANCE(peek(1))) {
                    runtimeError("Only instances of a class have fields.");
                    return INTERPRET_RUNTIME_ERROR;
                }
